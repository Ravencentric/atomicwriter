...
fn write_bytes(data: &[u8], dest: PathBuf, overwrite: bool) -> PyResult<()> {
...
#[pyfunction]
#[pyo3(signature = (data, dest, *, overwrite=false))]
fn write_text(data: &str, dest: PathBuf, overwrite: bool) -> PyResult<()> {
    write_bytes(data.as_bytes(), dest, overwrite)
}

#[pymodule(gil_used = false)]
fn atomicfile(m: &Bound<'_, PyModule>) -> PyResult<()> {
